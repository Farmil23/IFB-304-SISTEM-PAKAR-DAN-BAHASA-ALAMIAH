...
def sistem_pakar():
    ...
    print("="*60)
    print("Silakan jawab 'Y' jika Anda mengalami gejala di bawah ini,")
    print("atau 'T' jika tidak mengalami.\n")
    ...
    for g_kode, g_nama in semua_gejala.items():
        while True:
            ...
            jawaban = input(f"Apakah Anda mengalami [{g_kode}] {g_nama}? (Y/T): ").strip().upper() 
            
            if jawaban in ['Y', 'T']:
                break # KELUAR LOOPING WHILE INI AGAR NANTI DICEK DI FOR LOOP
            print("Input tidak valid! Harap masukkan 'Y' atau 'T'.")

        if jawaban == 'Y':
            # DISINI AKU NAMBAHIN SETIAP GEJALA USER JIKA JAWABANNYA Y KE GEJALA YANG DIALAMI DAN MEMASUKKAN G KODE AJA
            gejala_dialami.append(g_kode)
    ...
    for p_kode, p_data in knowledge_base.items():
        ...
        # TEKNIK DFS
        # G USER = ISI DARI GEJALA DIALAMI YAITU [ G01, ETC]
        for g_user in gejala_dialami:
            if g_user in p_data["gejala"]: 
                ...
                cf_terkumpul.append(p_data["gejala"][g_user]["cf_pakar"])
